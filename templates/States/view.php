...
<div class="view card card-primary card-outline">
  <div class="card-header d-sm-flex">
    <h2 class="card-title"><?= h($state->name) ?></h2>
  </div>
  ...
  <div class="card-footer d-flex">
    ...
    <div class="ml-auto">
      <?= $this->Html->link(__('Edit'), ['action' => 'edit', $state->id], ['class' => 'btn btn-secondary']) ?>
...
<div class="related related-locals view card">
  <div class="card-header d-sm-flex">
    <h3 class="card-title"><?= __('Related Locals') ?></h3>
    <div class="card-toolbox">
      <?= $this->Html->link(__('New'), ['controller' => 'Locals' , 'action' => 'add'], ['class' => 'btn btn-primary btn-sm']) ?>
      <?= $this->Html->link(__('List '), ['controller' => 'Locals' , 'action' => 'index'], ['class' => 'btn btn-primary btn-sm']) ?>
    </div>
  </div>
  <div class="card-body table-responsive p-0">
    <table class="table table-hover text-nowrap">
      ...
      <?php if (empty($state->locals)) { ?>
        <tr>
            <td colspan="4" class="text-muted">
              Locals record not found!
            </td>
        </tr>
      <?php }else{ ?>
        <?php foreach ($state->locals as $locals) : ?>
        <tr>
            <td><?= h($locals->id) ?></td>
            <td><?= h($locals->state_id) ?></td>
            <td><?= h($locals->name) ?></td>
            <td class="actions">
              <?= $this->Html->link(__('View'), ['controller' => 'Locals', 'action' => 'view', $locals->id], ['class'=>'btn btn-xs btn-outline-primary']) ?>
              <?= $this->Html->link(__('Edit'), ['controller' => 'Locals', 'action' => 'edit', $locals->id], ['class'=>'btn btn-xs btn-outline-primary']) ?>
              <?= $this->Form->postLink(__('Delete'), ['controller' => 'Locals', 'action' => 'delete', $locals->id], ['class'=>'btn btn-xs btn-outline-danger', 'confirm' => __('Are you sure you want to delete # {0}?', $locals->id)]) ?>
            </td>
        </tr>
        <?php endforeach; ?>
      <?php } ?>
